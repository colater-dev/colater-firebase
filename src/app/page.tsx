...
import { useAuth, useUser, initiateGoogleSignIn, initiateSmartGoogleSignIn, handleRedirectResult } from '@/firebase';
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    debugKeys.forEach(key => {
      ...
      if (value) {
        ...
        if (key === 'auth-debug-redirect' && parsed.urlParams) {
      ...
      handleRedirectResult(auth)
        .then(result => {
          if (result) {
            ...
            console.log("No redirect result to process - this is normal");
          }
        })
        .catch(error => {
  ...
  const handleSignIn = () => {
    ...
    if (auth) {
      console.log('Auth instance available, initiating smart Google sign-in');
      // Use smart authentication that adapts to the environment
